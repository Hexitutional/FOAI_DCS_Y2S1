...
Goal test   : len(state) == 8 (a state is only ever enqueued if it is
              already conflict-free, so reaching length 8 always means
              a valid, complete solution).
Frontier    : a First-In-First-Out (FIFO) queue, so BFS explores all
              nodes at a given depth before moving to the next depth
              level. This guarantees that the first solution found is
              the shallowest (optimal in terms of number of moves).
...
BFS instead uses the "incremental" formulation described above, and
always starts from an EMPTY board, so a conflicting full permutation
can never appear as a node in BFS's own search tree. To still put
each of the 10 datasets to genuine use, this program uses each one to
set the COLUMN TRIAL ORDER that BFS follows at every row of that run
(instead of always trying columns 0, 1, 2, ... 7, the run for Set N
tries columns in the order Set N lists them). This is still standard
...
through the same search tree, so the node counts and timings reported
below are meaningfully different and comparable across runs.
------------------------------------------------------------------
"""
...
def bfs_solve(column_order=None):
    """
    Breadth-First Search for a single Eight-Queen solution.
    ...
    Returns (solution, nodes_expanded):
        ...
        nodes_expanded - number of states popped off the queue, used
                         as BFS's search-effort metric.
    """
    ...
    while queue:
        ...
        if len(state) == BOARD_SIZE:
            return state, nodes_expanded         # goal test passed

        # Generate children for the next row in the given column order,
        # enqueuing only ones that don't conflict (pruned at generation time).
...
def main():
    ...
    for set_id, dataset in DATA_SETS.items():
        ...
        solution, nodes_expanded = bfs_solve(column_order=dataset)
        elapsed_ms = (time.perf_counter() - start_time) * 1000

        print(f"\nBFS solution                  : {solution}")
