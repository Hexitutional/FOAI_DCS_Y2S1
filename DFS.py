...
------------------------------------------------------------------
Formulation (see Report Section 4.2 "Depth-First Search (DFS)")
------------------------------------------------------------------
...
Goal test   : len(state) == 8 (a state is only ever pushed onto the
              stack if it is already conflict-free, so reaching length
              8 always means a valid, complete solution).
...
DFS instead uses the "incremental" formulation described above, and
always starts from an EMPTY board, so a conflicting full permutation
can never appear as a node in DFS's own search tree. To still put
each of the 10 datasets to genuine use, this program uses each one to
set the COLUMN TRIAL ORDER that DFS follows at every row of that run
...
def dfs_solve(column_order=None):
    """
    Depth-First (backtracking) search for a single Eight-Queen solution.
    ...
    Returns (solution, nodes_expanded):
        ...
        nodes_expanded - number of states popped off the stack, used
                          as DFS's search-effort metric.
    """
...
def main():
    ...
    for set_id, dataset in DATA_SETS.items():
        ...
        solution, nodes_expanded = dfs_solve(column_order=dataset)
